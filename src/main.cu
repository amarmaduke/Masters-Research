...
int main()
{
	...
	checkError(cudaMalloc(&d_p,sizeof(parameter)));
	...
	checkError(cudaMemcpy(d_p,&p,sizeof(parameter),cudaH2D));

	time_t start = time(0);
	vector<triple> grid
		= eulers_method(x,y,s,t,t_end,step,2,p,d_p);
	time_t end = time(0);
